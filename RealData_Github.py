...
# ---- Hessian handling ---------------------------------------------------
# The uploaded CORE applies floor=0 inside BY_exp/BY_orig, i.e. the FLOOR
# rule (zero h_i, keep g_i). The primary specification of the study is
# REJECT (zero both g_i and h_i where h_i < 0), which is what makes results
# independent of tree_method. CORE cannot express Reject through its `floor`
# argument, so it is applied here as a daughter-side wrapper. CORE untouched.
HESSIAN_RULE = "reject"    # "reject" (primary) | "floor" | "raw"

# ---- probability links --------------------------------------------------
# "linear" uses p_hat = (clip(s, -1, 1) + 1) / 2, the link under which
#     Modified Huber is a proper composite loss. This is the correct one.
# "blank" reports NaN, matching the current stopgap in the simulation tables.
# The tension: "linear" is right, but if the simulation tables still blank
# Modified Huber's probability columns, the two studies will disagree about
# whether Modified Huber has a Brier score at all. Pick one and apply it to
# both. Flagged rather than silently resolved.
...
METRICS = (
    #"Brier",
    "Accuracy",
    # "LogLoss",    # NOT safe for Modified Huber under the linear link: the
                    # number is driven by CLIP, not by the model. See the
                    # note in evaluate_real before switching this on.
    # "AUC",        # link-free, so the only metric on which hinge and
                    # Modified Huber can be ranked against the probability
                    # losses without asserting a link for them.
)

CLIP = 1e-12               # probabilities clipped to [CLIP, 1 - CLIP]. Only
                           # matters for LogLoss, and there it matters a lot:
                           # see the note in evaluate_real.
